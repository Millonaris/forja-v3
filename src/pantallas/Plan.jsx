...
import { CUT, HIDRATACION, HORA_GYM, MENSAJES, NOMBRE_FASE, NOMBRE_TIPO_DIA, PLANTILLAS_COMIDAS, PRIORIDADES, PROHIBICIONES, PROTEINA_RANGO, SUPLEMENTOS, TDEE_ESTIMADO } from "../datos/config.js";
...
const TEXTO_ACCION = { MANTENER: "Mantener", ESPERAR: "Esperar", CONSIDERAR_MENOS: "Considerar −100 a −150 kcal", CONSIDERAR_MAS: "Considerar +100 a +150 kcal", MANUAL_REVIEW_REQUIRED: "Revisión manual" };

export default function Plan({ r, ajustes, ir, abrirModal, avisar }) {
  const enCut = r.fase === "CUT" || r.fase === "PRE_CUT";
  const [plantilla, setPlantilla] = useState(r.tipoDia === "SOCIAL" ? "STRENGTH" : r.tipoDia || "STRENGTH");
  const o = r.objetivosDia;
  ...
  return (
    <>
      ...
      <Marco style={{ padding: 16, gap: 8 }}>
        <div className="entre"><div className="kicker">Dieta {r.fase === "CUT" || r.fase === "PRE_CUT" ? "del cut" : `de ${NOMBRE_FASE[r.fase].toLowerCase()}`}</div><div className="p12 tenue">Objetivo diario, no examen</div></div>
        {enCut ? (
          <>
            <div className="num" style={{ fontSize: 46 }}>{n0(CUT.semanaReferenciaKcal)} <span className="unidad" style={{ fontSize: 18 }}>kcal/semana</span></div>
            <div className="p13 medio">Semana estándar: 2 descanso + 3 fuerza + 2 social ≈ {n0(CUT.mediaReferenciaKcal)} kcal/día. Las calorías variables son por adherencia y reparto de combustible, no por ventaja metabólica. Hoy: <strong style={{ color: "var(--texto)" }}>{NOMBRE_TIPO_DIA[r.tipoDia]} · {n0(r.kcal)} kcal</strong>.</div>
            <div className="rejilla-3" style={{ gap: 8, textAlign: "center" }}>
              {["REST", "STRENGTH", "SOCIAL"].map((t) => (
                <div key={t} className="caja" style={{ padding: "10px 4px", borderColor: r.tipoDia === t ? "var(--acento)" : undefined }}>
                  <div className="etiqueta">{NOMBRE_TIPO_DIA[t]}</div>
                  <div className="num num-28">{n0(o[t].kcal)}</div>
                  <div className="p12 medio">{o[t].proteinaG} P{o[t].carbosG != null ? ` · ${o[t].carbosG} C · ${o[t].grasaG} G` : " · C/G flexibles"}</div>
                </div>
              ))}
            </div>
            <div className="p12 tenue">Proteína {PROTEINA_RANGO.min}–{PROTEINA_RANGO.max} g está bien. En social no se exigen macros: la estimación de restaurante es ruidosa. Suelo automático: {n0(CUT.sueloKcalAutomatico)} kcal (por debajo, revisión manual).</div>
          </>
        ) : (
          <>
        ...
        <div className="p13 medio">No se añaden kcal por gym o CaCo. No se comen “las calorías del Garmin”. {r.nutricion.diasDesdeCambio < 0 ? `Objetivo vigente desde el ${fechaMedia(ajustes.ultimoCambioKcal)}.` : `Último cambio: ${fechaMedia(ajustes.ultimoCambioKcal)} (hace ${r.nutricion.diasDesdeCambio} días).`}</div>
        <div className="caja columna" style={{ gap: 6 }}>
          <div className="p13"><strong>Sugerencia de FORJA: {TEXTO_ACCION[sug.accion] || sug.accion}.</strong> {sug.motivo}</div>
      ...
      <Marco style={{ padding: 16, gap: 8 }}>
        ...
        {!tdee.valido ? <div className="p12 tenue">Punto de trabajo ~{n0(TDEE_ESTIMADO.trabajo)} kcal/día, deliberadamente conservador (~10.000 pasos, taller pequeño, sin actividad laboral intensa).</div> : null}
      ...
      <div className="entre"><div className="kicker tenue">Plantillas de comidas · sugerencia, no regla</div><div className="p12 tenue">gym {HORA_GYM}</div></div>
      <div className="segmentos">{["STRENGTH", "REST", "SOCIAL"].map((t) => <button key={t} className={plantilla === t ? "activo" : ""} onClick={() => setPlantilla(t)}>{NOMBRE_TIPO_DIA[t]}</button>)}</div>
      {plantilla === "SOCIAL" ? (
        <div className="caja columna" style={{ gap: 6 }}>
          <div className="p13"><strong>Antes de la cena:</strong> {PLANTILLAS_COMIDAS.SOCIAL.antes}.</div>
          <div className="p13"><strong>Cena:</strong> {PLANTILLAS_COMIDAS.SOCIAL.cena}</div>
          <div className="p12 tenue">Registro rápido en el cierre del día: tamaño (800 / 1.200 / 1.600), bebidas y confianza. Si dudas entre dos, el superior. {MENSAJES.diaSocialSiguiente}</div>
        </div>
      ) : (
        <div className="lista">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 44px 44px 44px 56px", gap: 6, padding: "6px 0", borderBottom: "1px solid var(--linea)" }} className="etiqueta"><div>Comida</div><div className="der">P</div><div className="der">C</div><div className="der">G</div><div className="der">kcal</div></div>
          {PLANTILLAS_COMIDAS[plantilla].map((m) => (
            <div key={m.comida} style={{ display: "grid", gridTemplateColumns: "1fr 44px 44px 44px 56px", gap: 6, padding: "9px 0", borderBottom: "1px solid var(--linea)", alignItems: "center" }}>
              <div><div className="p14" style={{ fontWeight: 500 }}>{m.comida}</div><div className="p12 tenue">{m.hora}</div></div>
              <div className="der t" style={{ fontSize: 18 }}>{m.p}</div><div className="der t" style={{ fontSize: 18 }}>{m.c}</div><div className="der t" style={{ fontSize: 18 }}>{m.g}</div><div className="der t acento" style={{ fontSize: 18 }}>{m.kcal}</div>
            </div>
          ))}
          <div className="p12 tenue" style={{ padding: "6px 0" }}>Total {PLANTILLAS_COMIDAS[plantilla].reduce((t, m) => t + m.kcal, 0)} kcal · {PLANTILLAS_COMIDAS[plantilla].reduce((t, m) => t + m.p, 0)} P · {PLANTILLAS_COMIDAS[plantilla].reduce((t, m) => t + m.c, 0)} C · {PLANTILLAS_COMIDAS[plantilla].reduce((t, m) => t + m.g, 0)} G</div>
        </div>
      )}
      ...
      <div className="columna">
        ...
        <div className="caja"><div className="titulo-s">Comidas sociales</div><div className="p13 medio">{MENSAJES.comidaSocial} Sin ayuno punitivo, sin cardio de castigo y sin recorte automático al día siguiente.</div></div>
        <div className="caja"><div className="titulo-s">Suelo de 2.150 kcal</div><div className="p13 medio">FORJA no reduce sola ningún objetivo diario por debajo de 2.150 kcal. Si el algoritmo lo pidiera, se marca revisión manual.</div></div>
